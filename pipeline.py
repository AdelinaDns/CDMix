...
def _batch_fc(fc: ee.FeatureCollection) -> list:
    ...
    fc_sorted = fc.sort(PARCEL_ID_PROP)  # determinist între rulări
    batches = []
    for start in range(0, total, BATCH_SIZE):
        batch = ee.FeatureCollection(fc_sorted.toList(BATCH_SIZE, start))
...
def run_gee_export():
    ...
    # Ce e deja în bucket — nu re-submitem
    from collections import defaultdict

    client_gcs  = _get_gcs_client()
    bucket_obj  = client_gcs.bucket(GCS_BUCKET)
    blobs       = list(bucket_obj.list_blobs(prefix=GCS_PREFIX))
    existing    = set()
    for blob in blobs:
        rel = blob.name[len(GCS_PREFIX):].lstrip("/")
        existing.add(rel)
    log.info("Fișiere deja în bucket: %d", len(existing))
    ...
    for composite_idx, (window_idx, start, end) in enumerate(windows):
        ...
        for batch_idx, batch_fc in enumerate(batches):
            expected = f"date_{composite_idx:02d}/batch_{batch_idx:04d}.csv"
            if expected in existing:
                continue  # deja în bucket — skip
...
def run_assembly():
    ...
    Streaming mode — one CSV at a time, no pd.concat → no OOM.
    Resume support:
        - .assembly_progress  : CSV-uri deja procesate
        - .assembly_checkpoint.pkl : parcel data acumulat până acum
        - DATA/*.npy + sizes.json  : parcele deja salvate sunt sărite
    ...
    log.info("=== Stage 3: Assembly (Streaming + Resume) ===")
    csv_dir   = Path(OUTPUT_PATH) / "_csv_staging"
    all_paths = sorted([str(p) for p in csv_dir.rglob("*.csv")])

    data_dir.mkdir(parents=True, exist_ok=True)
    (Path(OUTPUT_PATH) / "META").mkdir(parents=True, exist_ok=True)

    # ── Resume: CSV progress ──────────────────────────────────────────────────
    progress_file = Path(OUTPUT_PATH) / ".assembly_progress"
    done_csvs     = set()
    if progress_file.exists():
        with open(progress_file) as f:
            done_csvs = set(line.strip() for line in f if line.strip())
        log.info("Resume: %d/%d CSVs deja procesate — continuând...",
                 len(done_csvs), len(all_paths))

    paths_todo = [p for p in all_paths if p not in done_csvs]
    log.info("CSVs rămase: %d", len(paths_todo))

    # ── Resume: load parcel data checkpoint ───────────────────────────────────
    checkpoint_file = Path(OUTPUT_PATH) / ".assembly_checkpoint.pkl"
    parcel_data     = {}
    # parcel_data[pid] = {"dates": {didx: arr(C,S) uint16}, "label": int|None}

    if checkpoint_file.exists() and len(done_csvs) > 0:
        log.info("Se încarcă checkpoint-ul...")
        with open(checkpoint_file, "rb") as f:
            parcel_data = pickle.load(f)
        log.info("Checkpoint: %d parcele în memorie.", len(parcel_data))

    # ── Resume: load existing META ────────────────────────────────────────────
    meta_sizes_path  = Path(OUTPUT_PATH) / "META" / "sizes.json"
    meta_labels_path = Path(OUTPUT_PATH) / "META" / "labels.json"
    sizes  = {}
    labels = {l: {} for l in LABEL_NAMES}

    if meta_sizes_path.exists():
        with open(meta_sizes_path) as f:
            sizes = json.load(f)
        log.info("Resume: %d parcele deja asamblate în META.", len(sizes))
    if meta_labels_path.exists():
        with open(meta_labels_path) as f:
            labels = json.load(f)

    CHECKPOINT_EVERY     = 50
    csv_since_checkpoint = 0

    # ── Stream CSVs ───────────────────────────────────────────────────────────
    for path in tqdm(paths_todo, desc="Streaming CSVs"):
        try:
            df = pd.read_csv(path, low_memory=False)
        except Exception as e:
            log.warning("Failed to read %s: %s — skip", path, e)
        ...
        df[PARCEL_ID_PROP] = df[PARCEL_ID_PROP].astype(str)
        df["date_idx"]     = df["composite_date"].map(DATE_TO_IDX)

        for pid, grp in df.groupby(PARCEL_ID_PROP):
            if pid not in parcel_data:
                parcel_data[pid] = {"dates": {}, "label": None}

            for didx_raw, date_grp in grp.groupby("date_idx"):
                if pd.isna(didx_raw):
                    continue
                didx = int(didx_raw)
                sub  = date_grp[S2_BANDS].dropna(how="any")
                if len(sub) == 0:
                    continue
                arr = np.clip(
                    sub.values.T.astype(np.float32), 0, 65534
                ).astype(np.uint16)  # (C, S_t)

                # Concatenează cu pixelii existenți pentru această dată
                if didx in parcel_data[pid]["dates"]:
                    parcel_data[pid]["dates"][didx] = np.concatenate(
                        [parcel_data[pid]["dates"][didx], arr], axis=1
                    )
                else:
                    parcel_data[pid]["dates"][didx] = arr

            # Label din primul rând valid
            if parcel_data[pid]["label"] is None:
                row = grp.iloc[0]
                for l in LABEL_NAMES:
                    if l in row and pd.notna(row[l]):
                        parcel_data[pid]["label"] = int(row[l])

        # Marchează CSV ca procesat
        with open(progress_file, "a") as f:
            f.write(path + "\n")

        # Checkpoint periodic
        csv_since_checkpoint += 1
        if csv_since_checkpoint >= CHECKPOINT_EVERY:
            log.info("Salvare checkpoint (%d parcele în memorie)...",
                     len(parcel_data))
            with open(checkpoint_file, "wb") as f:
                pickle.dump(parcel_data, f)
            csv_since_checkpoint = 0

    # Checkpoint final
    log.info("Salvare checkpoint final...")
    with open(checkpoint_file, "wb") as f:
        pickle.dump(parcel_data, f)

    # ── Build .npy per parcelă ────────────────────────────────────────────────
    skipped = 0

    for pid, pdata in tqdm(parcel_data.items(), desc="Building .npy"):
        # Skip dacă e deja asamblat dintr-un run anterior
        if (data_dir / f"{pid}.npy").exists() and pid in sizes:
            continue

        date_pixels = pdata["dates"]

        # S fixat de la prima dată validă — matching original format
        S = None
        for didx in range(N_DATES):
            if didx in date_pixels:
                S = date_pixels[didx].shape[1]
                break
        if S is None:
            skipped += 1
        ...
        arr = cp.asnumpy(out) if CUPY_AVAILABLE else out
